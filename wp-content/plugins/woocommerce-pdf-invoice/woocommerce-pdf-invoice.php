...
class WooPDF
{
    ...
    public function make_invoice($order)
    {
        ...
        RightPress_WC::order_add_meta_data($order, '_woo_pdf_invoice_date', time(), true);
        $order->save();
    }
    ...
    public function admin_invoice_link($content, $order)
    {
        ...
        // Show proforma link
        else if (!is_array($invoice) && $this->opt['woo_pdf_proforma_enabled'] && !$order->is_paid()) {
            $download_url = home_url('/?wpd_proforma=' . $order_id);
    ...
    public function user_invoice_link($order)
    {
        ...
        if (is_array($invoice) && !empty($invoice) && $this->opt['woo_pdf_enabled'] && $this->opt['woo_pdf_allow_download'] && apply_filters('woo_pdf_allow_regular_invoice_download', true, $order, 'single')) {
            $data = $invoice['id'].'|'.$invoice['prefix'].'|'.$invoice['code'].'|'.$invoice['suffix'];
            $download_code = base64_encode($data);
            $download_url = home_url('/?wpd_invoice=' . $download_code . '&order_key=' . $order_key);
        ...
        // Show proforma link
        else if (!is_array($invoice) && $this->opt['woo_pdf_proforma_enabled'] && $this->opt['woo_pdf_allow_proforma_download'] && !$order->is_paid() && apply_filters('woo_pdf_allow_proforma_invoice_download', true, $order, 'single')) {
            $download_url = home_url('/?wpd_proforma=' . $order->get_id() . '&order_key=' . $order_key);
    ...
    public static function get_order_invoice_path($order)
    {
        ...
        // Get proforma invoice path
            ...
            // Get (temporary) proforma invoice path
            $proforma_path = $this->get_proforma($order->get_id());
