...
abstract class Base {
	...
	public function do_send( $action_name, $arguments ) {
		...
		$this->logger->debug(
			...
			[ $action_name, json_encode( $arguments ) ]
		);
	...
	public function has_feeds() {
		...
		$this->logger->debug( '%s active feed(s) found for trigger %s.', [ count( $feeds ), $this->trigger_title ] );
	...
	protected function send_to_zapier( $action_name, $arguments ) {
		...
		foreach ( $this->get_feeds_for_this_trigger() as $feed ) {
			...
			$this->logger->debug( '  - %sJSON Data: %s...', [ $sampletext, substr( $json_data, 0, 72 ) ] );
			...
			if ( 200 !== wp_remote_retrieve_response_code( $result ) ) {
				...
				$this->logger->error( '%s for URL %s', [ $error_message, $webhook_url ] );
				$this->logger->debug( "args: %s\nresponse: %s\n", [ json_encode( $args ), json_encode( $result ) ] );
	...
	public function __call( $action_name, array $arguments ) {
		...
		$this->logger->debug( '%s::__call() : %s', [ get_called_class(), $this->trigger_title ] );
		...
		if ( false === $pos ) {
			...
			if ( isset( $_SERVER['HTTP_HOST'] ) && isset( $_SERVER['REQUEST_URI'] ) ) {
				...
				$this->logger->debug( '  - URL: %s%s', [ $_SERVER[ HTTP_HOST ], $_SERVER[ REQUEST_URI ] ] );
			}
