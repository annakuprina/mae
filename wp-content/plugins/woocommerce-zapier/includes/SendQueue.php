...
class SendQueue {
	...
	public function add_to_queue( Base $trigger, $action_name, $arguments ) {
		if ( ! $this->is_in_queue( $trigger, $action_name, $arguments ) ) {
			...
			$this->logger->debug(
				...
				[ $key, $trigger->get_trigger_key(), json_encode( $arguments ), $action_name ]
			);
			return $key;
		}
		return false;
	...
	public function process_queue() {
		...
		$this->logger->debug( 'Processing queue during %s. Queue contains %s task(s)...', [ $hook_name, $num_items ] );
		...
		$this->logger->debug( 'Queue processing for %s task(s) completed in %s seconds.', [ $num_items, $time_elapsed_secs ] );
		return $num_items;
	}
}
